...
impl WeatherClient {
    ...
    fn format_weekly_forecast(&self, forecast: &ForecastResponse) -> String {
        ...
        for (date, (day_name, forecasts)) in days {
            ...
            result.push_str(&format!("🌡 Температура: {:.1}°C (ощущается как {:.1}°C)\n", 
                (min_temp + max_temp) / 2.0, 
                avg_feels_like
            ));
            
            result.push_str(&format!("🔸 Мин: {:.1}°C, Макс: {:.1}°C\n", min_temp, max_temp));
            result.push_str(&format!("💧 Влажность: {:.0}%\n", avg_humidity));
            result.push_str(&format!("🍃 Ветер: {:.1} м/с, направление: {}\n", avg_wind_speed, wind_direction));
